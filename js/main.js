...
let processRaw = raw => {
    ...
    raw.forEach(country => {data[country.name] = country});
    ...
    const render = method => {
        ...
        const resetRegion = () => {
          ...
          d3.select(".grad-bar").style("background",
          `linear-gradient(to right,${style.interpolation(0.2)},${style.interpolation(0.5)},${style.interpolation(0.9)})`);
        };
        resetRegion();
        ...
        const svg = d3.select("svg-frame").append("svg").attr("viewBox", [170, -70, 630, 550]) // Global
    ...
    let decades = {
      two_thousands: {
        formula: dProp => {
          ...
          let val=Number(data[nm].data[rendering.genre][rendering.time]["count"])/
            (Number(data[nm].data[rendering.genre][rendering.time]["population"])/1000000)*10;
          return val;
          }
        },
        dataDefault: 0,
    ...
    */
    let methods = {
      ...
      metal: { //rendering.interp -> interpolationMaps[rendering.genre]
        formula: dProp => {
...
fetch(rawData).then(res => res.json()).then(processRaw)